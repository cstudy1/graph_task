...
class RotatE(nn.Module):
    def __init__(self, model_name, nentity, nrelation, args):
        ...
        self.entity_dim = self.hidden_dim * 2  # RotatE用复数，实体向量维度*2
        self.relation_dim = self.hidden_dim
        self.entity_embedding = nn.Embedding(nentity, self.entity_dim)
    ...
    def init_parameters(self):
        nn.init.uniform_(self.entity_embedding.weight.data, -self.gamma.item(), self.gamma.item())
        nn.init.uniform_(self.relation_embedding.weight.data, 0, 2 * math.pi)
    ...
    def _calc(self, h, r, t):
        # h, t: [batch, 2*dim] 复数表示，r: [batch, dim] 角度
        re_head, im_head = torch.chunk(h, 2, dim=1)
        re_tail, im_tail = torch.chunk(t, 2, dim=1)
        phase_relation = r
        re_relation = torch.cos(phase_relation)
        im_relation = torch.sin(phase_relation)
        re_pred = re_head * re_relation - im_head * im_relation
        im_pred = re_head * im_relation + im_head * re_relation
        re_score = re_pred - re_tail
        im_score = im_pred - im_tail
...
class ConvE(nn.Module):
    def __init__(self, model_name, nentity, nrelation, args):
        ...
        self.args = args
        self.hidden_dim = args.hidden_dim
        self.embedding_shape1 = 10
        self.embedding_shape2 = self.hidden_dim // self.embedding_shape1
        self.entity_embedding = nn.Embedding(nentity, self.hidden_dim)
        self.relation_embedding = nn.Embedding(nrelation, self.hidden_dim)
        self.conv1 = nn.Conv2d(1, 32, (3, 3), 1, 1, bias=True)
        self.bn0 = nn.BatchNorm2d(1)
        self.bn1 = nn.BatchNorm2d(32)
        self.bn2 = nn.BatchNorm1d(self.hidden_dim)
        self.dropout = nn.Dropout(args.lmbda)
        self.fc = nn.Linear(self.embedding_shape1 * self.embedding_shape2 * 64, self.hidden_dim)
        self.init_parameters()
    ...
    def _calc(self, h, r, t):
        batch_size = h.size(0)
        h = h.view(-1, 1, self.embedding_shape1, self.embedding_shape2)
        r = r.view(-1, 1, self.embedding_shape1, self.embedding_shape2)
        stacked_inputs = torch.cat([h, r], 2)  # [batch, 1, 2*shape1, shape2]
