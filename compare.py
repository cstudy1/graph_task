...
def main():
    args = Args()
    data = load_dataset('cora').to(args.device)
    layers_list = [2, 3, 4]
    lr_list = [0.01, 0.005, 0.001]
    model_names = list(MODELS.keys())
    records = []
    total_combinations = len(model_names) * len(layers_list) * len(lr_list)
    current_comb = 0
    print(f'Starting hyperparameter search for Cora dataset. Total combinations: {total_combinations}')
    for model_name in model_names:
        for num_layers in layers_list:
            for lr in lr_list:
                current_comb += 1
                print(f'\n========== Combination {current_comb}/{total_combinations}: {model_name} layers={num_layers} lr={lr} ==========')
                ...
                for epoch in range(1, args.epochs + 1):
                    ...
                    if val_acc > best_val_acc:
                        ...
                        best_results = {k: v.copy() for k, v in results.items()}
                        best_epoch = epoch
                        is_new_best = True
                    print(f'Epoch {epoch:03d}/{args.epochs}: loss {train_loss:.4f} time {elapsed:.2f}s val_acc {val_acc:.4f}' + (' (new best)' if is_new_best else ''))
                print(f'Finished training. Best epoch: {best_epoch}, best val acc: {best_val_acc:.4f}, test acc: {best_results["test"]["acc"]:.4f}')
                # Save best results
                for split, metrics in best_results.items():
                    row = {
                        'model': model_name,
                        'num_layers': num_layers,
                        'lr': lr,
    ...
    out_file = f"result/node_full_Cora_hyperparam_results.xlsx"
