...
def get_args():
    ...
    p.add_argument('--fanouts', type=lambda x: list(map(int, x.split(','))), default='15,10')
...
# ---------- 采样器 ----------
def make_loader(data, fanouts, batch_size):
    train_idx = data.train_mask.nonzero(as_tuple=False).view(-1)
    return NeighborLoader(data, num_neighbors=fanouts, batch_size=batch_size,
...
# ---------- 训练 ----------
def train_once(model, loader, optimizer, device):
    model.train()
...
def run_model(model_name, data, args):
    ...
    loader = make_loader(data, args.fanouts, args.batch_size)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)

    epoch_times = []
    for epoch in range(1, args.epochs + 1):
        start_time = time.time()
        train_loss = train_once(model, loader, optimizer, args.device)
...
def main():
    ...
    for name in dataset_names:
        ...
        for mname in MODELS:
            print(f'\n========== {mname} ==========')
            results, epoch_times = run_model(mname, data, args)
        ...
        out_file = f"result/node_sample_{name}_results.xlsx"
