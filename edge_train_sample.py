...
# ---------- 训练 ----------
def train_once(model, data, optimizer, device, neg_ratio):
    model.train()
    ...
    for dname in dataset_names:
        ...
        for mname in MODELS:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                loss = train_once(model, train_data, optimizer, args.device, args.neg_ratio)
        ...
        for split_name, res in [('val', val_res), ('test', test_res)]:
            row = {'model': mname, 'split': split_name, **res}
            records.append(row)

    # 保存 Excel
    df = pd.DataFrame(records)
    out_file = f"result/link_sample_{args.dataset}_results.xlsx"
